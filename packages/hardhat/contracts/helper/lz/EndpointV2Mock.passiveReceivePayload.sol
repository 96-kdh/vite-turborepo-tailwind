...
contract EndpointV2MockPassiveReceivePayload is ILayerZeroEndpointV2, MessagingContext {
    ...
    // Struct to store delayed messages.
    struct DelayedMessage {
        Origin origin;
        address endpoint;
        address receiver;      // Destination address (converted from bytes32)
        bytes32 payloadHash;
        bytes message;
        uint256 gas;
        uint256 msgValue;
    ...
    DelayedMessage[] public messageQueue;
    ...
    function send(
        ...
        DelayedMessage memory dm = DelayedMessage({
            origin: origin,
            endpoint: lzEndpoint,
            receiver: packet.receiver.bytes32ToAddress(), // convert bytes32 to address
            payloadHash: payloadHash,
            message: receiveMessage,
            gas: totalGas,      // Example gas value (adjust as needed)
            msgValue: dstAmount,      // For simplicity, set to zero here (adjust if needed)
            guid: packet.guid  // Dummy guid; in practice, compute as required.
        });
        messageQueue.push(dm);
    }

    function releaseQueuedMessages() external {
        for (uint i = 0; i < messageQueue.length; i++) {
            DelayedMessage storage dm = messageQueue[i];
            EndpointV2MockPassiveReceivePayload(dm.endpoint).receivePayload{ value: dm.msgValue }(
                dm.origin,
                dm.receiver,
                dm.payloadHash,
                dm.message,
